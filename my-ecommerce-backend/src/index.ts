...
async function upsertPermission(strapi: Strapi, roleId: number, action: string) {
  const q = strapi.db.query('plugin::users-permissions.permission');
  const existing = await q.findOne({ where: { action, role: roleId } });
  if (existing) {
    if (!existing.enabled) await q.update({ where: { id: existing.id }, data: { enabled: true } });
    return;
  }
  await q.create({ data: { action, enabled: true, roles: [{ id: roleId }] } });
}
...
async function grantPublicRead(strapi: Strapi) {
  ...
  for (const a of [
    ...
    'api::product.product.find',  'api::product.product.findOne',
...
async function upsertPublished(strapi: Strapi, uid: string, where: any, data: any) {
  const svc = doc(strapi, uid);
  const existing = await svc.findFirst({ params: { filters: where, publicationState: 'preview' } });
  if (existing) {
...
async function runSeed(strapi: Strapi) {
  ...
  const store = await upsertPublished(strapi, 'api::store.store',
    { slug: storeSlug },
    { name: storeName, slug: storeSlug, domain: 'neomercado.com.br', description: 'Loja principal do NeoMercado', active: true }
  );
  strapi.log.info(`Store OK: ${store.name}`);

  // 2) Categories
  const catDefs = [
    { name: 'Eletrônicos', description: 'Tecnologia e gadgets', featured: true },
    { name: 'Moda',        description: 'Vestuário e acessórios', featured: false },
  ];
  const cats: Record<string, any> = {};
  for (const c of catDefs) {
    const slug = slugify(c.name);
    const cat = await upsertPublished(strapi, 'api::category.category',
      { slug },
  ...
  const prodDefs = [
    {
      ...
      price: 199.99, stock: 30, featured: true, active: true,
    ...
    },
    {
      ...
      price: 39.9, stock: 50, featured: false, active: true,
  ...
  for (const p of prodDefs) {
    ...
    const prod = await upsertPublished(strapi, 'api::product.product',
      { slug },
      {
        name: p.name,
        slug,
  ...
  for (const pg of pages) {
    ...
    const page = await upsertPublished(strapi, 'api::page.page',
      { slug },
  ...
  const count = async (uid: string) => strapi.db.query(uid).count({ where: {} });
  ...
  strapi.log.info(`Resumo seed → stores:${cStores} categories:${cCats} products:${cProds} pages:${cPages}`);
}
